...
import com.nostra13.universalimageloader.utils.ImageFileUtil;
...
final class LoadAndDisplayImageTask implements Runnable, IoUtils.CopyListener {
    ...
    public void run() {
        ...
        try {
            ...
            String gifPath = configuration.memoryFilePathCache.get(memoryCacheKey);
            if (bmp == null || bmp.isRecycled() || !TextUtils.isEmpty(gifPath)) {
                bmp = tryLoadBitmap();
                if (bmp == null && imageLoadingInfo.mFileTypeStatus != BitmapDisplayer.FILE_TYPE_GIF_IMAGE)
                ...
                if (bmp != null && options.isCacheInMemory()) {
                    ...
                    if (imageLoadingInfo.mFileTypeStatus == BitmapDisplayer.FILE_TYPE_GIF_IMAGE) {
                        configuration.memoryFilePathCache.put(memoryCacheKey, imageLoadingInfo.mFilePath);
                    }
                }
            } else {
    ...
    private Bitmap tryLoadBitmap() throws TaskCancelledException {
        ...
        try {
            ...
            if (imageFile != null && imageFile.exists() && imageFile.length() > 0) {
                ...
                checkTaskNotActual();
                imageLoadingInfo.mFilePath = imageFile.getAbsolutePath();
                if (ImageFileUtil.isGifImageByFile(imageFile.getAbsolutePath())) {//发现图片是gif图片
                    imageLoadingInfo.mFileTypeStatus = BitmapDisplayer.FILE_TYPE_GIF_IMAGE;
                } else {
                    imageLoadingInfo.mFileTypeStatus = BitmapDisplayer.FILE_TYPE_NORMAL_IMAGE;
                }
